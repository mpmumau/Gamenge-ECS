...
TEST(EntityManagerTest, construct)
{
    ...
    for (EID eid = 0; eid < ECS_MAX_ENTITIES; eid++) {
        ...
        EXPECT_EQ(Mask(), entities[eid].mask);
...
TEST(EntityManagerTest, addGetMask)
{
    ...
    EID eid = entityManager.addEntity();
    Mask mask = Mask(0x01 << 17 | 0x01 << 12 | 0x01 << 3);

    entityManager.addMask(eid, mask);
    EXPECT_EQ(mask, entityManager.getMask(eid));
    EXPECT_EQ(Mask(0x01 << 17 | 0x01 << 12 | 0x01 << 3), entityManager.getMask(eid));
    EXPECT_FALSE(MaskUtils::matches(Mask(0x01 << 17), entityManager.getMask(eid)));
    EXPECT_TRUE(MaskUtils::matches(Mask(0x01 << 17 | 0x01 << 12 | 0x01 << 3), entityManager.getMask(eid)));
    EXPECT_TRUE(MaskUtils::matchesAny(Mask(0x01 << 12), entityManager.getMask(eid)));
    EXPECT_TRUE(MaskUtils::matchesAny(Mask(0x01 << 3), entityManager.getMask(eid)));
    EXPECT_TRUE(MaskUtils::matchesAny(Mask(0x01 << 17), entityManager.getMask(eid)));
    EXPECT_FALSE(MaskUtils::matchesAny(Mask(0x01 << 4 | 0x01 << 13), entityManager.getMask(eid)));
    EXPECT_TRUE(MaskUtils::matchesAny(Mask(0x01 << 3 | 0x01 << 12), entityManager.getMask(eid)));
    EXPECT_TRUE(MaskUtils::matchesAny(Mask(0x01 << 17), entityManager.getMask(eid)));
}
...
TEST(EntityManagerTest, removeMask)
{
    ...
    Mask component1 = Mask(0x01 << 2);
    Mask component2 = Mask(0x01 << 17);
    Mask component3 = Mask(0x01 << 18);
    Mask component4 = Mask(0x01 << 31);
